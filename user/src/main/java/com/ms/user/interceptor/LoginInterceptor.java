...
public class LoginInterceptor  extends HandlerInterceptorAdapter {
    public static ThreadLocal<Map<String, UmsMember>> threadLocal = new ThreadLocal<>();
    @Autowired
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if(log.isInfoEnabled()){
            ...
            log.info("用户访问地址: {}, 来路地址: {}", uri, ipAddr);
        }
    /*    UmsMember userInfo = getUserInfo(token);
        ...
        HashMap<String, UmsMember> map = new HashMap<>();
        map.put("admin",member);
        threadLocal.set(map);
        return true;
    }
